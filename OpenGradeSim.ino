...
bool gradeIsSet = false;          // use to exit manual mode. false = manual mode , true = automatic mode
...
double Kp_avg = 1.0, Ki_avg = 0, Kd_avg = 44.3;
...
int riderWeight = 113;            // variable for combined rider and bike weight
...
bool wheelIsSet = false;         // note whether the wheel cm setting is done
...
int wheelCircCM = 2070;           // Wheel circumference in centimeters (700c 32 road wheel) 2300
...
void getBLEServices() {
  ...
  while (!cablePeripheral.connected()) {
    ...
    //OLED.displayLineLeft(0, F"BLE SCANNNING");
    //OLED.displayLineLeft(1, F("for CABLE Device")
    //
    //    OLED.setTextSize(1);
...
bool autoLevelTrainerIncline() {
  ...
  switch (sampleTimes)
  {
    ...
    default:
      ...
      bargraph(0, 27, sampleTimes * 2.56, 6);
      //return false;
      return pressAnyButtonToExit();
      break;
    case 50:
...
bool setWeight(void)
{
  return setNumber(riderWeight, 1, 1000, 1, F(" kg"));
}

bool setWheelSize(void)
{
  return setNumber(wheelCircCM, 1, 1000, 1, F(" cm"));
}

bool setGrade(void)
{
  return setNumber(wheelCircCM, 1, 1000, 1, F(" %"));
}
...
{
  // Read the content of "userSettings_FlashStore"
  userSettings = userSettings_FlashStore.read();

  // Intitialize flash store for the first time.
  if (userSettings.valid == false) {
    Serial.println("Initializing User Settings Flash Storage");
    userSettings.weight = riderWeight; // use default value
    userSettings.valid = true;
    //userSettings_FlashStore.write(userSettings);
  }

  riderWeight = userSettings.weight;
}

void UpdateUserWeightSettings()
{
  userSettings.weight = riderWeight;
  //userSettings_FlashStore.write(userSettings);
}

void updateUserWheelSettings()
{
